...
Test to check whether seqprep can process NextSeq sample sheet with one dummy library entry.
'''
...
class NSNoentrySampleSheet(unittest.TestCase):
	...
	#self.testdatadir = "/n/informatics/git/testdata"
	...
	self.jobcmd1 = "seqprep --primary /n/informatics/git/testdata --suffix '_test_dummyentry' --verbose  150529_NS500305_0015_AHFCWWBGXX"
        #pass
	...
	os.system("cp /n/informatics/git/testdata/150529_NS500305_0015_AHFCWWBGXX/SampleSheet.dummyentry.csv /n/informatics/git/testdata/150529_NS500305_0015_AHFCWWBGXX/SampleSheet.csv")
	...
	samplesheet_path = "%s/150529_NS500305_0015_AHFCWWBGXX_test_dummyentry/SampleSheet.csv" % (self.pathfinished)
	...
	if os.path.exists(samplesheet_path):
		...
		#print output
	if output > 0:
		print("\nSampleSheet Dummy Entry Test succeeded. Wrote out SampleSheet of size: %s" % (output))
	else:
		print("\nSampleSheet Dummy Entry Test failed")
