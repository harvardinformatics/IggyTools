from iggytools.iggyseq.seqmem.seqmemUtil import diskPercentUsage, ageInDays, runNameMatch
from iggytools.utils.util import email
    ...
    for item in os.listdir(primaryDir):
        ...
        if runNameMatch(item):
            ...
            if elapsedDays > maxDaysInPrimary:  #move run to archive folder
                ...
                if verbose: 
                    print '  Moving %s to %s (elapsed: %s days)' % (runPath, newRunPath, elapsedDays)
...
def reportDiskUsage(dirList, emailList):
    message = ''
    ...
    if message:
        email(emailList, 'SeqMem Warning: disk usage > 90%', message)
